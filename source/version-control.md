...
如果文件成功推送到了 GitHub 上的项目仓库，你会看到一条成功提示（{numref}`git-push-03`）。点击“关闭（Dismiss）”即可继续在 Jupyter 中工作。
...
本章所讲内容的练习题，可以在配套的[练习册仓库](https://worksheets.python.datasciencebook.ca)中“Collaboration with version control（借助版本控制协作）”一行找到。点击“查看练习册（view worksheet）”，可以预览本章练习册的非交互版本。要交互式地完成这些习题，请按照练习册仓库中的说明下载全部练习册，并按照{numref}`第 %s 章 <move-to-your-own-machine>`中给出的计算机配置说明操作。这样才能确保练习册提供的自动反馈和指导按预期工作。
