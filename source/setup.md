...
- 如果计算机上的管理员账户与你的用户账户不同，就必须把该用户加入“docker-users”组。以管理员身份运行计算机管理（Computer Management），打开本地用户和组（Local Users and Groups）→ 组（Groups）→ `docker-users`。在 `docker-users` 上右键单击，把该用户加入该组。注销后重新登录，更改才会生效。
