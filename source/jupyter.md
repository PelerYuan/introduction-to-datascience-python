...
代码单元格既可以单独运行，也可以作为运行整个笔记本的一部分来执行——后者要用到 Jupyter 的**运行**（Run）菜单或**内核**（Kernel）菜单里的“**运行全部单元格**（Run All Cells）”命令。单独运行一个代码单元格，通常是在编辑或编写自己的 Python 代码时采用的做法。运行整个笔记本，通常是为了在把分析分享给别人之前确认它能完整运行，另外也用于把笔记本纳入自动化流程的场合。

要单独运行某个代码单元格，先要激活它：用光标点击该单元格即可。Jupyter 会在单元格左侧用一个蓝色矩形框把它高亮出来，表示已经激活。单元格激活后（{numref}`activate-and-run-button`），既可以按工具栏上的**运行**（Run）（&#x23F5;）按钮来运行，也可以使用键盘快捷键 `Shift + Enter`。

```{figure} img/jupyter/activate-and-run-button-annotated.png
---
name: activate-and-run-button
---
一个已激活、可以运行的单元格。红色箭头指向单元格左侧的蓝色矩形框。蓝色矩形框表示这个单元格可以运行了。点击**运行**（Run）按钮（用红圈标出）即可运行。
```
...
---
点击**重启内核并运行全部单元格……（Restart Kernel and Run All Cells...）**，即可重启 Python 会话。
```
...
1. 点击屏幕顶部的**内核**（Kernel）菜单，再点击**中断内核**（Interrupt Kernel）。
2. 如果还是不行，点击**内核**（Kernel）菜单，再点击**重启内核**（Restart Kernel...）。这样做之后，你必须从笔记本开头开始，把代码单元格一直运行到你上次停下的位置。
3. 如果仍然不行，就重启 Jupyter。先点击屏幕左上角的**文件**（File）菜单，再点击**保存笔记本**（Save Notebook），保存好你的工作。接下来，如果你是通过 JupyterHub 服务器访问 Jupyter，就在**文件**（File）菜单中点击 **Hub 控制面板**（Hub Control Panel）。选择**停止我的服务器**（Stop My Server）把它关掉，再点击**我的服务器**（My Server）按钮重新启动。如果你是在自己的电脑上运行 Jupyter，就在**文件**（File）菜单中点击**关闭**（Shut Down），然后重新启动 Jupyter。最后，回到你刚才在用的笔记本。
...
要编辑 Jupyter 中的 Markdown 单元格，需要双击该单元格。双击之后，会显示未格式化的文本，也就是*未渲染*（unrendered）的版本（{numref}`markdown-cell-not-run`）。然后就可以用键盘编辑文本了。要查看格式化（即*已渲染*）后的文本（{numref}`markdown-cell-run`），点击工具栏上的**运行**（Run）（&#x23F5;）按钮，或使用 `Shift + Enter` 快捷键。
...
要在 Jupyter 中新建 Markdown 单元格，点击工具栏上的 `+` 按钮。Jupyter 中所有新建单元格默认都是代码单元格，所以必须改变单元格格式，它才会被识别为 Markdown 单元格并渲染出来。做法是：用光标点击该单元格，确认它已经激活。然后点击工具栏上标着“**代码**（Code）”的下拉框（它就在 &#x23ED; 按钮旁边），把它从“**代码**（Code）”改成“**Markdown**”（{numref}`convert-to-markdown-cell`）。
...
和对待任何正在处理的文件一样，一定要经常保存，免得丢失已完成的进度！Jupyter 有自动保存（autosave）功能，会定期保存打开的文件，默认每两分钟保存一次。你也可以手动保存 Jupyter 笔记本：在**文件**（File）菜单中选择**保存笔记本**（Save Notebook），点击工具栏上的磁盘图标，或者使用快捷键（Windows 上是 `Control + S`，macOS 上是 `Command + S`）。
...
读到这里，你大概已经知道（至少能想到），Jupyter 笔记本非常擅长交互式地编辑、编写和运行 Python 代码——它们本来就是为此设计的！因此，Jupyter 笔记本在代码单元格的执行顺序上很灵活。这种灵活性意味着，用**运行**（Run）（&#x23F5;）按钮可以按任意顺序运行代码单元格。但这种灵活性也有代价：可能让笔记本的代码无法按线性顺序（从笔记本顶部到底部）执行。非线性笔记本会带来麻烦，因为代码文档习惯上都按线性顺序运行，别人运行你的笔记本时也会这样期待。最后，如果代码要用在某种自动化流程中，就必须按线性顺序、从笔记本顶部到底部运行。
...
name: out-of-order-2
---
乱序编写，并用**运行**（Run）按钮以非线性顺序成功运行的代码。执行顺序可以顺着代码单元格左侧的编号看下去；这些编号的顺序就表示单元格的运行顺序。
```
...
name: out-of-order-3
---
乱序编写，并用“**重启内核并运行全部单元格……（Restart Kernel and Run All Cells...）**”按线性顺序运行的代码。结果第二个代码单元格运行时出错，没能把笔记本中的全部代码单元格都运行完。
```
...
我们建议的最佳实践是：在每一段工作中，至少在全新的 Python 会话里完整运行整个笔记本 2–3 次。请注意，这一点很关键：你*必须在全新的 Python 会话中做这件事*，也就是要重启内核。我们建议使用**内核**（Kernel） >> **重启内核并运行全部单元格……（Restart Kernel and Run All Cells...）**，或者工具栏上的 &#x23ED; 按钮。注意，**运行**（Run） >> **运行全部单元格**（Run All Cells）菜单项不会重启内核，所以它不足以防范这类错误。
...
2. 在 Jupyter 笔记本中写代码的过程中，经常按线性顺序完整运行笔记本（每段工作 2–3 次），做法是使用 Jupyter 菜单中的**内核**（Kernel） >> **重启内核并运行全部单元格……（Restart Kernel and Run All Cells...）**，或者工具栏上的 &#x23ED; 按钮。
...
总有一天，你会想为自己的项目新建一个全新的 Jupyter 笔记本，而不是查看、运行或编辑别人创建的笔记本。做法是：切换到**启动器**（Launcher）标签页，点击**笔记本**（Notebook）标题下的 Python 图标。如果看不到**启动器**（Launcher）标签页，可以点击 Jupyter 文件浏览器顶部的 **+** 按钮新建一个（{numref}`launcher`）。

```{figure} img/jupyter/launcher-annotated.png
---
name: launcher
---
点击**笔记本**（Notebook）标题下的 Python 图标，就会新建一个使用 Python 内核的 Jupyter 笔记本。
```
