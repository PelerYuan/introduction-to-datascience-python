...
- $n$ 是观测个数，
- $y_i$ 是 $i^\text{th}$ 个观测的观测值，
- $\hat{y}_i$ 是 $i^\text{th}$ 个观测的预测值。
...
K 近邻回归模型的预测以三维空间中的曲面表示，叠加在使用三个预测变量（价格、房屋面积和卧室数量）的数据之上（译注：原文这里说用了三个预测变量，但价格是响应变量，预测变量实际只有房屋面积和卧室数量两个）。一般我们并不推荐使用三维可视化；这里只是为了教学演示，才用三维可视化展示预测曲面是什么样子。
```
